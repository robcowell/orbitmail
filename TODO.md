...
- **imapflow 1.4.3 → 2.0.5, which clears the last `npm audit` finding.** 1.4.3
  ...
  nodemailer's `http-proxy-client`, and every advisory is in the address parser,
  `resolveContent` or the recipient-domain checks — so the GitHub alerts
  (#67–#70) had been dismissed as not used. **The earlier entry recording this
  was wrong about the fix**: it said 1.5.0 was the last 1.x and still pinned
  nodemailer, so only 2.x would clear it. 1.x runs to 1.7.8, and imapflow
  dropped nodemailer in **1.6.0**, so `npm update` inside `^1` would have done.
  The mistake came from reading only the last line of a multi-version
  `npm view imapflow@1` — the listing is not sorted newest-last. Worth
  checking the changelog before calling a major bump the only route.
  2.x was taken anyway: its breaking changes are Node ≥ 20 (Electron 44 ships
  24) and dropping `lib/` from the package, and nothing here imports past the
  package root. It also brings the 1.6–1.7 fixes, among them IMAP command
  injection (1.6.4), hardened parsing of untrusted server responses (1.6.6) and
  credential masking in the raw log (1.7.1). **GreenMail does not exercise the
  IMAP4rev2 handling** those releases reworked (2.0.1 and 2.0.2 are both
...
- **mailparser 3.9.16 → 3.9.28, to move its nodemailer off a high-severity
  ...
  mailparser's subtree. The copy inside imapflow was not fixed by this; see the
  imapflow entry above. Verified with `build`, `test:pure`, `test:db`, `test:store` and
