...
- **Sent mail never filed for manual IMAP accounts.** `smtp-send.ts:140` guards `appendToSentFolder` on `if (info.message)`, but nodemailer's SMTP transport info object has no `message` property — dead branch. Gmail/O365 mask it by auto-appending server-side; a self-hosted account's sent message exists nowhere.
