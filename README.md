...
**How mail is rendered.** Message HTML comes from whoever sent it, so it is sanitized before display: scripts, forms, frames and embedded objects are removed, and CSS that would let a message paint over the app is stripped. **Remote images are blocked by default** — a message's external images and CSS backgrounds are not fetched until you ask, because loading them confirms you read the mail and reveals your IP to the sender. A bar above the message offers **Load images** (just this message) or **Always load from _sender_** (remembered per sender); embedded and inline images still show. The window itself cannot be navigated away from the app, and a Content-Security-Policy backs both. Links open in your browser, not in Orbit Mail. Attachments whose type can execute — `.desktop`, `.sh`, `.exe` and similar — ask for confirmation before opening, naming the real file extension.
