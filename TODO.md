...
- `buildLikePattern` leaves `_` as a live LIKE wildcard (`search-index.ts:33-37`) — `\w` keeps the underscore, so a search for `foo_bar` also matches `fooXbar`. (The former companion finding here, `SEARCH_FIELD_COLUMNS['constructor']`, is stale — that symbol no longer exists in the code.)
