...
- ~~**Packaged builds can't do OAuth.**~~ **Fixed** (#45) — credentials resolve at runtime from the environment or `~/.config/orbit-mail/.env`, so a packaged app can sign in without the project `.env` that dotenv's cwd lookup cannot find. Deliberately **not** solved by baking them into the build: that would ship the builder's own client secret in every package (CLAUDE.md rule 5, enforced by a test). **Still open:** entering credentials in-app when adding an account, so a packaged build does not require editing a file.
